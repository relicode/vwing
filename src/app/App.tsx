...
import OnlineGame from '$/app/OnlineGame'
import PracticeGame from '$/app/PracticeGame'
...
type Route =
  ...
  | { view: 'lobby' }
  | { view: 'practice' }
  | { view: 'online'; game: string; pilot: string; intent: JoinIntent }
    ...
    <ThemeProvider theme={theme}>
      ...
      >
        ...
        >
          ...
          {route.view === 'lobby' ? (
            <LobbyScreen
              onJoin={(game, pilot, intent) => setRoute({ view: 'online', game, pilot, intent })}
              onBack={toTitle}
            />
          ) : null}
          {route.view === 'practice' ? <PracticeGame weapon={weapon} onExit={toTitle} /> : null}
          {route.view === 'online' ? (
            <OnlineGame game={route.game} pilot={route.pilot} intent={route.intent} onExit={toLobby} />
          ) : null}
        </Box>
      </Box>
    </ThemeProvider>
